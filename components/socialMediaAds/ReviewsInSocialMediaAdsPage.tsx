"use client";

import { useState, useEffect } from "react";
import TestimonialCard from "../aboutUs/TestimonialCard";

type Testimonial = {
    text: string;
    name: string;
    role: string;
    image: string;
};

const testimonials: Testimonial[] = [
    {
        text: "The social media strategy Performd implemented for us was a game-changer. Our engagement tripled in just two months.",
        name: "Peter Braun",
        role: "Business Owner",
        image: "/coreValues/balance.png",
    },
    {
        text: "I was skeptical about social media ads, but the ROI we've seen since working with this team is undeniable.",
        name: "Anna Smith",
        role: "Marketing Director",
        image: "/coreValues/hand.png",
    },
    {
        text: "Highly professional and data-driven. They don't just post content; they build a community around your brand.",
        name: "John Carter",
        role: "Entrepreneur",
        image: "/coreValues/Openness.png",
    },
    {
        text: "Their ad creatives are stunning. We've seen a significant drop in our CAC since we started our partnership.",
        name: "Lisa Wong",
        role: "Startup Founder",
        image: "/coreValues/planet.png",
    },
];

export default function ReviewsInSocialMediaAdsPage() {
    const [index, setIndex] = useState(0);
    const [visibleSlides, setVisibleSlides] = useState(4);

    const nextSlide = () => {
        setIndex((prev) => (prev + 1) % testimonials.length);
    };

    const prevSlide = () => {
        setIndex((prev) => (prev - 1 + testimonials.length) % testimonials.length);
    };

    // Function to detect slides per view
    const updateVisibleSlides = () => {
        if (window.innerWidth < 640) {
            setVisibleSlides(1);
        } else if (window.innerWidth < 1024) {
            setVisibleSlides(2);
        } else if (window.innerWidth == 1024) {
            setVisibleSlides(3);
        } else {
            setVisibleSlides(4);
        }
    };

    useEffect(() => {
        updateVisibleSlides();
        window.addEventListener("resize", updateVisibleSlides);
        return () => window.removeEventListener("resize", updateVisibleSlides);
    }, []);

    return (
        <section className="py-12 relative overflow-hidden">
            <div className="w-full px-4 sm:px-8 lg:px-16">
                <h2 className="text-center text-2xl sm:text-4xl font-bold mb-8">
                    Results our clients love.
                </h2>

                <div className="flex flex-col justify-between">
                    {/* Card Container */}
                    <div className="flex gap-4 sm:gap-6 transition-transform duration-500 mb-6">
                        {Array.from({ length: Math.min(visibleSlides, testimonials.length) }).map((_, i) => {
                            const testimonial =
                                testimonials[(index + i) % testimonials.length];
                            return (
                                <div key={i} className="flex-1 min-w-[250px]">
                                    <TestimonialCard
                                        text={testimonial.text}
                                        name={testimonial.name}
                                        role={testimonial.role}
                                        image={testimonial.image}
                                    />
                                </div>
                            );
                        })}
                    </div>

                    {/* Arrows */}
                    <div className="flex justify-center sm:justify-end gap-3">
                        <button
                            onClick={prevSlide}
                            className="bg-white w-10 h-10 flex items-center justify-center rounded-full shadow hover:bg-secondaryBackground"
                        >
                            ←
                        </button>
                        <button
                            onClick={nextSlide}
                            className="bg-white w-10 h-10 flex items-center justify-center rounded-full shadow hover:bg-secondaryBackground"
                        >
                            →
                        </button>
                    </div>
                </div>
            </div>
        </section>
    );
}
