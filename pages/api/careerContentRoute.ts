...
async function handleGet(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (id) {
    ...
    const record = await Career.findOne({ _id: id, isDeleted: false }).lean();
    if (!record) {
  ...
  const records = await Career.find({ isDeleted: false }).lean();
...
async function handlePost(req: NextApiRequest, res: NextApiResponse) {
  ...
  await dbConnect();
  const created = await Career.create(parsed.data);
