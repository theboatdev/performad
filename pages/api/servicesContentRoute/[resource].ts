...
import Services from "@/contentManagementSystem/models/Services";
import {
  BusinessPillarSchema,
  DontDoItemSchema,
  SeoFeatureSchema,
  ServiceCardSchema,
} from "@/contentManagementSystem/schemas/services.schema";

interface ResourceConfig {
  field: "serviceCards" | "seoFeatures" | "businessPillars" | "dontDoItems";
  schema: ZodObject<ZodRawShape>;
}

const resourceConfig: Record<string, ResourceConfig> = {
  serviceCards: { field: "serviceCards", schema: ServiceCardSchema },
  seoFeatures: { field: "seoFeatures", schema: SeoFeatureSchema },
  businessPillars: { field: "businessPillars", schema: BusinessPillarSchema },
  dontDoItems: { field: "dontDoItems", schema: DontDoItemSchema },
};
...
export default async function handler(
  ...
  try {
    ...
    console.error("Services resource API error:", error);
...
async function handleGet(
  ...
  const servicesId = getServicesId(req, res);
  if (!servicesId) {
    return;
  }
  ...
  const record = await Services.findOne({ _id: servicesId, isDeleted: false })
    .select(config.field)
    .lean();

  if (!record) {
    return res.status(404).json({ error: "Services entry not found" });
  }
  ...
  const found = collection.find((entry: any) => entry?._id?.toString() === itemId);
...
async function handlePost(
  ...
  const updated = await Services.findOneAndUpdate(
    { _id: servicesId, isDeleted: false },
...
function getServicesId(req: NextApiRequest, res: NextApiResponse) {
  const servicesId = req.query.servicesId;
  if (!servicesId || Array.isArray(servicesId)) {
    res.status(400).json({ error: "Missing servicesId" });
    return null;
  }
  return servicesId;
}
